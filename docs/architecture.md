...
Pointers are native integers into unmanaged memory or stack storage. Globals and constant aggregates are allocated before their initializers and relocations are installed. Function-pointer tables use `ldftn`; portable and legacy indirect calls use managed `calli`. System-mode scalar pointers use unmanaged-callable thunks and Cdecl `calli`.
...
System-mode internal variadic callbacks use distinct unmanaged-callable identities registered against their managed implementations. Translated indirect variadic calls resolve these identities before using managed `calli` with the software argument area. Unknown native variadic pointers are rejected at runtime. Native invocation of a registered variadic identity terminates with an explicit diagnostic instead of misreading native registers as a managed argument buffer. Pointer identity, mixed register/stack arguments, formatting, and these boundary errors are tested under JIT and NativeAOT.
